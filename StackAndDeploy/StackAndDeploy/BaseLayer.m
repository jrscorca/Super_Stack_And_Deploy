...
- (id)init{
    if ((self = [super init])) {
        ...
        base.model.ownership = [[[Ownership alloc] initWithPlayerType:LOCAL_PLAYER] autorelease];
        base.position = ccp(MDM.boardSize.width/4, MDM.boardSize.height/2);
        base.scale = 1.5;
        
        BaseSprite *base2 = [[[BaseSprite alloc] init] autorelease];
        base2.model.ownership = [[[Ownership alloc] initWithPlayerType:AWAY_PLAYER] autorelease];
